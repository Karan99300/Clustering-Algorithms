...
Taken from: https://github.com/mgarrett57/DENCLUE/blob/master/denclue.py
"""
import numpy as np
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        for j1 in range(self.n_samples):
            for j2 in (x for x in range(self.n_samples) if x != j1):
                ...
                if diff <= (g_clusters.nodes[j1]['attr_dict']['radius']+g_clusters.nodes[j1]['attr_dict']['radius']):
        ...
        for num_clusters, clust_nodes in enumerate(clusters):
            clust_nodes = list(clust_nodes)

            # Get maximum density of attractors and location
            max_instance = max(clust_nodes, key=lambda x: g_clusters.nodes[x]['attr_dict']['density'])
            max_density = g_clusters.nodes[max_instance]['attr_dict']['density']
            max_centroid = g_clusters.nodes[max_instance]['attr_dict']['attractor']

            # Populate cluster_info dict
            cluster_info[num_clusters] = {'instances': clust_nodes,
                                           'size': len(clust_nodes),
            ...
            if max_density >= self.min_density:
                labels[clust_nodes] = num_clusters
            else:
                labels[clust_nodes] = -1
